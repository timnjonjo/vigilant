...
|---|---|---|---|
| `vigilant-dashboard` | public, PKCE S256 | authorization-code | React analyst console (redirect `http://localhost:5173/*`) |
...
(its own secret + hardcoded `tenant_id`). The dev secret here is **not** for
production.
...
## Roles & seed users (dev password: `password`)
...
## Get a token (for curl / testing the backend)

```bash
curl -s http://localhost:8081/realms/vigilant/protocol/openid-connect/token \
  -d grant_type=password -d client_id=vigilant-dashboard \
  -d username=analyst-loob -d password=password | jq -r .access_token
```
